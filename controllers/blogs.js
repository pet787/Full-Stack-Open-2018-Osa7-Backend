...
blogRouter.post('/', async (request, response) => {
  ...
  try {
    ...
    const blog = new Blog({ title, author, url, likes: (likes || 0), user: user._id, comments } )

    const result = await blog.save()

    const blogPopulated = await Blog
    .findById( result._id )
    .populate( 'user', { username: 1, name: 1 } )
...
blogRouter.post('/:id/comments', async (request, response) => {
  ...
  const comments = [...oldBlog.comments, comment ] 
  const blog = await Blog.findByIdAndUpdate(request.params.id, { comments }, {new: true} )
  response.send(blog)
})
